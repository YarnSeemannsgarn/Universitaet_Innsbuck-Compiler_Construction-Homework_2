PROGRAM Sort(input, output);
    ...
    PROCEDURE Quicksort(size: Integer; VAR arr: IntArrType);
        ...
        { This does the actual work of the quicksort.  It takes the parameters which define the range of the array to work on, and references the array as a global. }
        PROCEDURE QuicksortRecur(start, stop: integer);
            ...
            { The quicksort split algorithm.  Takes the range, and returns the split point. }
            FUNCTION Split(start, stop: integer): integer;
                ...
                BEGIN { Split }
                    { Set up the pointers for the hight and low sections, and get the pivot value. }
                    pivot := arr[start];
                    left := start + 1;
                    right := stop;
                    ...
                    { This is how you return function values in pascal. Yeccch. }
                    Split := right
                    # Wrong comment
                END;
